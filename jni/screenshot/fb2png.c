...
int get_device_fb(const char* path, struct fb *fb)
{
    ...
    if (ioctl(fd, FBIOGET_FSCREENINFO, &finfo) < 0) {
        printf("ioctl fixed failed, %s", strerror(errno));
        close(fd);
        return -1;
    }
    ...
    if (read_size < 0 || (unsigned)read_size != raw_size) {
        printf("%s : %s\n", "read buffer error", strerror(errno));
        goto oops;
    }
...
int fb2pngfb1(const char *path)
{
    struct fb fb;
    int ret;

#ifdef ANDROID
    ret = get_device_fb("/dev/graphics/fb1", &fb);
#else
    ret = get_device_fb("/dev/fb0", &fb);
#endif
...
int fb2pngfb2(const char *path)
{
    struct fb fb;
    int ret;

#ifdef ANDROID
    ret = get_device_fb("/dev/graphics/fb2", &fb);
#else
    ret = get_device_fb("/dev/fb0", &fb);
#endif
